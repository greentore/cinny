...
function initMarkdown(extPlainRules, extMarkdownRules) {
  pl.parser = parserFor(extPlainRules);
  pl.plainOut = outputFor(extPlainRules, 'plain');
  pl.htmlOut = outputFor(extPlainRules, 'html');

  md.parser = parserFor(extMarkdownRules);
  md.plainOut = outputFor(extMarkdownRules, 'plain');
  md.htmlOut = outputFor(extMarkdownRules, 'html');
}

/** Borrowed from simple-markdown. */
const LIST_LOOKBEHIND_R = /(?:^|\n)( *)$/;

function applyGreentextRules(_extPlainRules, extMarkdownRules) {
...
function extendRules() {
  ...
  rulePatches.forEach((rulePatch) => rulePatch(extPlainRules, extMarkdownRules));
